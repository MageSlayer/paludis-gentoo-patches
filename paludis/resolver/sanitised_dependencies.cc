        ...
        const std::shared_ptr<const ChangedChoices> & changed)
{
    ...
    else
    {
        ...
        if (id->run_dependencies_target_key())
            _populate_one(env, decider, resolution, id, changed, &PackageID::run_dependencies_target_key);
        if (id->run_dependencies_host_key())
            _populate_one(env, decider, resolution, id, changed, &PackageID::run_dependencies_host_key);
